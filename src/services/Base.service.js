...
class BaseService {
  ...
  async filter(data) {
    try {
      ...
      throw new Error(
        `Error ${error.message} while fetching entity by ${data}`
      );
    }
  }
  ...
  async update(req, res) {
    try {
      ...
      // If there are files attached to the request, upload and extract image filenames
      if (req.files && req.files.length > 0) {
        const uploadedImages = await this._uploadAndExtractImageFilenames(
          req,
          res
        );
  ...
  /*async _deleteAssociatedImages(id) {
    ...
    if (entity && entity.images && entity.images.length > 0) {
      ...
      await Promise.all(
        entity.images.map((filename) => this._deleteImage(filename))
      );
    }
  }*/
  ...
  /*async _deleteExistingImages(id) {
    ...
    if (
      ...
      existingEntity.images.length > 0
    ) {
      // Delete each image associated with the entity
      await Promise.all(
        existingEntity.images.map((filename) => this._deleteImage(filename))
      );
    }
  }
